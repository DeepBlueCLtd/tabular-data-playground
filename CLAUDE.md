...
8. `/speckit-implement` — execute `tasks.md`. Status → `implementing`,
   then `complete` when merged and the Definition of Done in `spec.md`
   §13 is met.
...
4. **Confirm the epic gate.** For E0 specifically, the Phase 0 gate in
   ...
   green.
5. **Branching strategy.** Default to **one feature branch per backlog
   item**, merged in dependency order. Only batch multiple items onto
   one branch if they are tightly coupled (e.g. tokeniser + parser +
   executor for the mini-shell, items #22–#24) and the user explicitly
   approves; record the rationale in the plan's Complexity Tracking.

## Per-item loop within the epic

For each item in the ordered list:

1. Run the per-item cycle above end-to-end.
2. Mark the corresponding TodoWrite todo `completed` immediately on
   merge — do not batch.
3. Update `backlog.md`: status column, `Updated` date, strikethrough
   on completion. Commit with `docs: backlog status — #<id> <status>`.
4. If `/speckit-analyze` or implementation surfaces a new sharp edge,
   stop and update `docs/limitations.md` in the same change
   (Principle VII).
5. If a downstream item's assumptions changed (e.g. Measurement C
...
1. Strikethrough the epic row in the `## Epics` table of
   `backlog.md`.
2. Open a single PR titled `epic: E<n> — <title>` summarising the
   merged items and linking the demonstrable artefact (deployed URL,
   recorded measurements, etc.).
3. Stop and hand back to the user before starting the next epic — do
